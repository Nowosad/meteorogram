...
dataset <- read.csv("http://openmeteo.pl/meta/wrf/poznan.txt", sep=";", na.strings=c("-999000000.00 ","  Entire grid contents are set to missing data "))
...
lines(x2, y2)
lines(dates,DF1$dpt2m/DF1$temp, xaxt='n', xlab='', type='l', col='coral', lwd=2)
abline(h=c(0,0.5,1,2,5,10), lty=3)
abline(v =seq(dates[1],max(dates), by="6 hour"), col="black", lty=3)


par(fig=c(0.10,0.90,0.30,0.47), new=TRUE, mar = c(0, 0, 0, 0))
slp_range <- round(range(DF1$cisn_msl))
slp_range[1] <- slp_range[1]-6
slp_range[2] <- slp_range[2]+6
barplot(DF1$cisn_msl, ylim=slp_range, col=add_slp_color(76),names.arg = dates, xaxt='n', yaxs='i',xaxs='i', border=NA, cex.axis=0.8,space = 0.0)
box()
